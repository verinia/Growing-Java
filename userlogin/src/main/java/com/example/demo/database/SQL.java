...
/*
             if (isBanned(target)) {
                MySQL.tryUpdate("UPDATE list SET until="+duration+", `by`='"+from+"', `reason`='"+msg+"' WHERE name='"+target+"'");
            } else {
                MySQL.tryUpdate("INSERT INTO list VALUES ('"+target+"', "+now+", "+duration+", '"+from+"', '"+msg+"')");
     *
     */
